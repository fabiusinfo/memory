...
bool Quality::repair(vector<Point3D> &p, vector<Element *> &e, 
					 Metric *metric, unsigned int &max_iter, const double &threshold){
	...
	for (unsigned int i=0;i<e.size();i++){
		...
		vector<double> node_jens = metric->getQuality(e[i]);
...
bool Quality::repairOne(vector<Point3D> &p, vector<Element *> &e,
	...
	for (auto elem: elemlist) {
		...
		double qua = metric->getQuality(e[std::get<0>(elem)])[std::get<1>(elem)];
		if (qua<threshold) {
			current_qua += qua;
			num_inv++;
