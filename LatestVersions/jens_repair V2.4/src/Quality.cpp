...
//--------------------------------------------------------------------------
//Esta función agrega los nodos pertenecientes a la superficie a una lista especial.
void Quality::InvPointSFChecker(vector<Point3D> &p, vector<Element *> &e, list<unsigned int> &inv_nodesS, FaceContainer &fc, unsigned  int &i,  unsigned  int &j,  map<int, list<int>> &neighborsmap){
...
//Repair them.
	...
	for (auto node: inv_nodes){
		...
		if (!repairOneAlt2(p,e,node,metric,max_iter,0, step, neighborsmap)) {
						valid = false;
					}
	}

	//Acá permite ir mejorando los nodos cada uno de a poco.
	//Para que funcione, se debe quitar el while dentro de repairOneX que itera hasta mejorar al nodo.
	/*
	...
	for (int i=0; i<max_iter; i++){
		...
		for (auto node: inv_nodes){
			...
			if (!repairOneAltX(p,e,node,metric,max_iter,threshold, 1, neighborsmap)) {
				valid = false;
			}
		...
		for (unsigned int i=0;i<e.size();i++){
			...
			while (worst<threshold) {
				...
				if (worst < threshold) {
					...
					if (!repairOneAlt2(p,e,e[i]->getPoints()[widx],metric,max_iter,threshold, step, neighborsmap)) {
						quality = false;
					}
					last_time = widx;
				}
...
bool Quality::repairOneAlt1(vector<Point3D> &p, vector<Element *> &e,
...
bool Quality::repairOneAlt2(vector<Point3D> &p, vector<Element *> &e,
						unsigned int &nodeidx, Metric *metric, 
						unsigned int &max_iter, const double &threshold, int step, map<int, list<int>> &neighborsmap) {
...
bool Quality::repairOneAlt3(vector<Point3D> &p, vector<Element *> &e,
						unsigned int &nodeidx, Metric *metric, 
						unsigned int &max_iter, const double &threshold, int step, map<int, list<int>> &neighborsmap) {
